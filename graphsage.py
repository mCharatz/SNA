...
class GraphSAGE(torch.nn.Module):
    ...
    def __init__(self, config, dim_in, dim_out):
        ...
        if config['architecture'] == 'two-layer':
            self.sage1 = SAGEConv(dim_in, config['dim_h1'], aggr=config['aggr'], project=config['proj'])
            self.sage2 = SAGEConv(config['dim_h1'], dim_out, aggr=config['aggr'], project=config['proj'])
        else:
            self.sage1 = SAGEConv(dim_in, config['dim_h1'], aggr=config['aggr'], project=config['proj'])
            self.sage2 = SAGEConv(config['dim_h1'], config['dim_h2'], aggr=config['aggr'], project=config['proj'])
            self.sage3 = SAGEConv(config['dim_h2'], dim_out, aggr=config['aggr'], project=config['proj'])
    ...
    def fit(self, data, stats):
        ...
        for epoch in range(1, self.config['epochs'] + 1):
            ...
            for batch in train_loader:
                ...
                f1 = f1_score(batch.y[batch.train_mask].cpu(), out[batch.train_mask].cpu().argmax(dim=1), average='micro')
                loss.backward()
                optimizer.step()
...
def grid_search_cv(data, cv, param_grid):
    """Performs Grid Search for the GraphSAGE model and keeps the best performing model based on the F1 score"""
                                    ...
                                    for dim_h1 in param_grid[i]['dim_h1']:
                                        for dim_h2 in param_grid[i]['dim_h2']:
                                            ...
                                            for cv_split in cv_splits:
                                                ...
                                                model = GraphSAGE(config, data.num_features, 3)
