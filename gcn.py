...
class GCN(torch.nn.Module):
    ...
    def __init__(self, config, dim_in, dim_out):
        ...
        if config['architecture'] == 'two-layer':
            self.gcn1 = GCNConv(dim_in, config['dim_h1'])
            self.gcn2 = GCNConv(config['dim_h1'], dim_out)
        else:
            self.gcn1 = GCNConv(dim_in, config['dim_h1'])
            self.gcn2 = GCNConv(config['dim_h1'], config['dim_h2'])
            self.gcn3 = GCNConv(config['dim_h2'], dim_out)
...
def grid_search_cv(data, cv, param_grid):
    """Performs Grid Search for the GCN model and keeps the best performing model based on the F1 score"""
                        ...
                        for dim_h1 in param_grid[i]['dim_h1']:
                            for dim_h2 in param_grid[i]['dim_h2']:
                                ...
                                for cv_split in cv_splits:
                                    ...
                                    model = GCN(config, data.num_features, 3)
